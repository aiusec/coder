...
func TestDeleteOldWorkspaceBuildOrchestrations(t *testing.T) {
	...
	assertOrchestrationDeleted(ctx, t, db, oldCompletedParent.ID)
	assertOrchestrationExists(ctx, t, db, oldFailedParent.ID, oldFailed.ID)
	assertOrchestrationExists(ctx, t, db, oldCanceledParent.ID, oldCanceled.ID)
	assertOrchestrationExists(ctx, t, db, oldPendingParent.ID, oldPending.ID)
	assertOrchestrationExists(ctx, t, db, recentCompletedParent.ID, recentCompleted.ID)
	...
	assertOrchestrationDeleted(ctx, t, db, oldFailedParent.ID)
	assertOrchestrationDeleted(ctx, t, db, oldCanceledParent.ID)
	assertOrchestrationExists(ctx, t, db, oldPendingParent.ID, oldPending.ID)
	assertOrchestrationExists(ctx, t, db, recentCompletedParent.ID, recentCompleted.ID)
}

func assertOrchestrationDeleted(ctx context.Context, t *testing.T, db database.Store, parentBuildID uuid.UUID) {
	t.Helper()

	_, err := db.GetWorkspaceBuildOrchestrationByParentBuildID(ctx, parentBuildID)
	require.ErrorIs(t, err, sql.ErrNoRows)
}

func assertOrchestrationExists(ctx context.Context, t *testing.T, db database.Store, parentBuildID uuid.UUID, orchestrationID uuid.UUID) {
	t.Helper()

	orchestration, err := db.GetWorkspaceBuildOrchestrationByParentBuildID(ctx, parentBuildID)
	require.NoError(t, err)
