...
func (r *RootCmd) aiGatewayKeysDelete() *serpent.Command {
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if err != nil {
				return xerrors.Errorf("parse AI Gateway key ID %q: %w", inv.Args[0], err)
			}

			_, err = cliui.Prompt(inv, cliui.PromptOptions{
				Text:      fmt.Sprintf("Are you sure you want to delete AI Gateway key with ID %s?", id),
				IsConfirm: true,
			...
			err = client.DeleteAIGatewayKey(inv.Context(), id)
			if err != nil {
				return xerrors.Errorf("delete AI Gateway key %q: %w", id, err)
			}

			_, _ = fmt.Fprintf(inv.Stdout, "Deleted AI Gateway key %s at %s.\n", cliui.Keyword(id.String()), cliui.Timestamp(time.Now()))
			return nil
		},
	}
