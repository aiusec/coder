...
func (r *RootCmd) openApp() *serpent.Command {
	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			// Sub-agent external app URLs are attacker-influenceable
			// through workspace configuration (e.g. devcontainer.json) and
			// runtime sub-agent registration. The provenance check above
			// already blocked $SESSION_TOKEN substitution for them. When
			// the URL still contains the literal placeholder it was
			// clearly trying to receive the token; refuse to hand it to
			// the OS open handler and let the user decide what to do.
			// Sub-agent URLs that don't reference the placeholder carry
			// no token to leak, so they open as before.
